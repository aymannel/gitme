...
At the heart of GitMe is the `gitme.zsh` script, a zsh script which overwrites the `README.md` file in a local repository with the most recent version in your Obsidian Vault. GitMe also acts as a git wrapper, and will `git add`, `git commit` and `git push` your `README.md` file after updating it.
